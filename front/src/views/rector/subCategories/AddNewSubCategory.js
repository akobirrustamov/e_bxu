...
import ApiCall from "../../../config"; // Assuming ApiCall is a function for making API requests
import {Link, useNavigate} from "react-router-dom";
import { useTransition } from 'react';
import {MdArrowBack} from "react-icons/md";
import * as MdIcons from "react-icons/md";
const MdAddToHomeScreen = lazy(() => import("react-icons/md").then(module => ({ default: module.MdAddToHomeScreen })));
...
function AddNewSubCategory() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (sabab.length === 1 && sabab[0].title === "") {
            newSubCategory.reasons = [];
        }

        console.log("Payload for submission:", newSubCategory);

        try {
            await ApiCall(`/api/v1/superadmin/subcategory`, "POST", newSubCategory);
    ...
    return (
        <div className="container mx-auto pt-6 p-32">
            <div className={"flex justify-between items-baseline"}>
                <h2 className="text-xl font-bold mb-4">Yangi Xizmat turi Qo'shish</h2>
                <Link to={"/superadmin/subcategories"} className="text-xl flex gap-1 items-center font-bold text-white mb-4 bg-red-500 p-1 rounded-xl"><MdArrowBack/> bekor qilish</Link>
            </div>
            <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4 bg-white p-6 rounded-xl">
                    ...
                    <div
                        className="mt-2 w-[80px] flex items-center justify-center rounded-full bg-lightPrimary p-[12px] text-5xl font-bold text-brand-500 dark:!bg-navy-700 dark:text-white">
                ...
                <div className="mb-4 flex items-baseline gap-2 bg-white p-6 rounded-xl">
                    ...
                    <label className="block mb-2">Arizaga Mas'ul javob bersin:</label>
                </div>
                {/*ariza*/}
                <div className="mb-4  bg-white p-6 rounded-xl">
                                            ...
                                            <div className={"w-full border-2   pb-4  shadow-cyan-700"}>
                                                <div className={"items-end w-full flex justify-end "}>
                                                    ...
                                                    <p className="w-2/3 lg:w-1/2  sm:w-2/3 md:w-2/3 text-sm text-gray-700 pt-2  ">
                                                        Buxoro davlat texnika universiteti rektori v.b.
                                                        S.B.Saidovga<b>_guruh_</b> guruh talabasi
                                                        <b>_talaba_ism_familya_</b> tomonidan
                                                    </p>
                                                </div>
