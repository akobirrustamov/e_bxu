...
const Teacher = () => {
  ...
  const getTeachers = async () => {
    try {
      const response = await ApiCall(`/api/v1/teacher`, "GET", null);
      console.log(response.data);
  ...
  const handleDeleteClick = (id) => {
    if (window.confirm("Ishonchiz komilmi")) {
      deleteTeacher(id);
    }
  };
      ...
      <Card extra={"w-full h-full shadow-sm"}>
        <div className="overflow-x-auto p-4">
          <table className="w-full min-w-max">
            ...
            <tbody>
              {teachers && teachers.map((row, index) => (
                <tr
                  key={index}
      ...
      >
        <div className="p-4">
          <h2 className="mb-6 text-xl font-bold text-gray-800">
            {editingTeacher ? "teacher tahrirlash" : "Yangi teacher qo'shish"}
          </h2>
          <form
            onSubmit={(e) => {
              e.preventDefault();
