...
function Speciality() {
  ...
  const handleUpdateSpeciality = async () => {
    try {
      setIsUpdating(true);
      await getSpecialityFromHemis(); // просто инициирует обновление
      await getSpeciality(); // получает свежие группы
      // const newResponse = await ApiCall(`/api/v1/Speciality/1`, "GET");
      // console.log(newResponse.data);
    } catch (error) {
      console.error("Xatolik (yangilash):", error);
      alert("Mutaxassislik yangilanmadi. Iltimos, qayta urinib ko‘ring.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const getSpeciality = async () => {
    try {
      const response = await ApiCall(`/api/v1/specialty`, "GET");
      console.log("specialty", response);

      const raw = response.data;
      const allSpeciality = Array.isArray(raw)
        ? raw
        : Array.isArray(raw?.data)
        ? raw.data
        : [];

      setSpeciality(allSpeciality);

      const uniqueDepartments = [
        ...new Set(allSpeciality.map((group) => group.structureType)),
      ];
      ...
      console.error("Xatolik (Mutaxassislar):", error);
    }
  };

  // Отправить запрос на обновление (не возвращает группы)
  const getSpecialityFromHemis = async () => {
    try {
      const response = await ApiCall(`/api/v1/specialty/update`, "GET");
  ...
  const filteredSpeciality = Array.isArray(speciality)
    ? speciality
        .filter((speciality) => speciality?.structureType === selectedDepartment)
        .filter((speciality) =>
          speciality?.name?.toLowerCase().includes(searchTerm.toLowerCase())
        )
    : [];

  const handleGroupClick = (specialityId) => {
    navigate(`/superadmin/specialty/${specialityId}`);
  };
      ...
      <div className="mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredSpeciality.map((speciality) => (
                <div
                  ...
                  onClick={() => handleGroupClick(speciality.id)}
                >
                  <div className="p-6">
            ...
            <div className="rounded-xl bg-white py-12 text-center shadow-sm">
              ...
              <p className="mx-auto mt-1 max-w-md text-gray-500">
                {searchTerm || selectedDepartment !== "Sirtqi bo'lim"
                  ? "Qidiruvga mos yoki tanlangan bo'limdagi Mutaxassislar mavjud emas"
                  : "Mutaxassislar ro'yxati hozircha bo'sh"}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 text-center">
          <p className="text-lg text-gray-700">
            Topilgan Mutaxassislar:{" "}
            <span className="text-2xl font-bold text-blue-600">
              {filteredSpeciality.length} ta
            </span>
          </p>
        </div>
      </div>
