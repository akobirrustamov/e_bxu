...
const Admins = () => {
  ...
  const updateAdmin = async () => {
    try {
      ...
      await ApiCall(
        `/api/v1/superadmin/admins/${editingAdmin.id}`,
        "PUT",
        updatedAdmin
      );
  ...
  const handleEditClick = (admin) => {
    ...
    setSelectedSubCategories(
      adminDuties
        .filter((duty) => duty.admin.id === admin.id)
        .map((duty) => ({
  ...
  return (
    <div>
      ...
      <div className="grid h-full grid-cols-1 gap-5 pt-0 pb-0 pr-32 md:grid-cols-1">
        <Card extra={"w-full h-full"}>
          <div className="overflow-x-scroll p-4 xl:overflow-x-hidden">
              ...
              <tbody>
                {admins.map((row, index) => (
                  <tr key={index} className={"border-b-2"}>
                    <td>{index + 1}</td>
                    <td>
                      <p className={"bold text-lg"}>{row.name}</p>
                    </td>
                    <td>
                      <p className={"bold text-lg"}>{row.phone}</p>
                    </td>
                    <td>
                      {adminDuties.length > 0 &&
                        adminDuties
                          .filter((item) => item.admin.id === row.id)
                          .map((item, subIndex) => (
                            <p
                              key={subIndex}
                              className="my-1 rounded-2xl bg-blue-100 p-2 pb-0 pt-0"
                            >
                              {subIndex + 1}. {item.subCategory.name}
                            </p>
                          ))}
                    </td>
                    <td className="flex">
      ...
      >
        <div className={"mb-8 "}>
          <h2 className="mb-4 text-lg font-bold">
            {editingAdmin ? "Admin tahrirlash" : "Admin qo'shish"}
          </h2>
          <form
            className={""}
            onSubmit={(e) => {
          ...
          >
            <div className="mb-4">
              <label className="mb-2 block">Ism Familya:</label>
              <input
                type="text"
            ...
            <div className="mb-4">
              <label className="mb-2 block">Phone/Login:</label>
              <input
                type="text"
            ...
            <div className="mb-4">
              <label className="mb-2 block">Vazifalari:</label>
              <Select
                isMulti
